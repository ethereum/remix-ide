...
class PluginManagerComponent extends ViewPlugin {
  ...
  renderItem (name) {
    ...
    const activationButton = isActive
      ? yo`
      <button onclick="${_ => this.appManager.deactivatePlugin(name)}" class="btn btn-secondary btn-sm">
        Deactivate
      </button>`
      : yo`
      <button onclick="${_ => this.appManager.activatePlugin(name)}" class="btn btn-success btn-sm">
        Activate
      </button>`
